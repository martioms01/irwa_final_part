...
def index():
    ...
    session['context_id'] = context_id # provisional - track user context
...
def search_form_post():
    ...
    session['last_search_id'] = search_id #provisional - track queries
...
def analytics_dashboard():
    click_counts = analytics_data.fact_clicks
    query_data = analytics_data.queries
    context_data = analytics_data.user_contexts
    print(context_data)

    # Prepare data for display
    click_stats = [{'doc_id': doc_id, 'click_count': len(clicks)} for doc_id, clicks in click_counts.items()]
    query_stats = [{'query_id': qid, 'terms': q['terms'], 'timestamp': q['timestamp']} for qid, q in query_data.items()]
    user_context_stats = [{'context_id': cid, **context} for cid, context in context_data.items()]

    return render_template('analytics.html', click_stats=click_stats, query_stats=query_stats, user_context_stats=user_context_stats)
# PROVISIONAL --------------------------------------------------------------------------------------------------------------------------
